...
import ReloadCam_CriptoBlock

def GetVersion():
   return 1

recvblock = ReloadCam_CriptoBlock.CryptographicBlock()
sendblock = ReloadCam_CriptoBlock.CryptographicBlock()
...
def TestCline(cline):
    ...
    try:
        ...
        try:
            ...
            #But we send "CCCam" with the password encripted ReloadCam_CriptoBlock
            cccamArray = GetCcam()
...
def DoHanshake(socket):
    import hashlib, array, ReloadCam_CriptoBlock

    random = bytearray(16)
    socket.recv_into(random, 16) #Receive first 16 "Hello" random bytes
    #print "Hello bytes: " + random

    random = ReloadCam_CriptoBlock.Xor(random); #Do a Xor with "CCcam" string to the hello bytes
